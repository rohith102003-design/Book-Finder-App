...
CURATED_SEED_BOOKS = [
    ...
    },
    {
        ...
        "edition_count": 80,
        "subjects": ["Dystopian", "Classics", "Political Fiction", "Totalitarianism", "Literature"],
    },
    {
    ...
    },
    {
        ...
        "edition_count": 12,
        "subjects": ["Programming", "Software Engineering", "Computer Science", "Technology", "Clean Code"],
    },
    {
        ...
        "edition_count": 95,
        "subjects": ["Classics", "Romance", "Literature", "Historical Fiction", "British"],
    },
    {
    ...
    },
    {
        ...
        "edition_count": 35,
        "subjects": ["Science Fiction", "Space Opera", "Classics", "Sci-Fi", "Galactic Empire"],
    },
    {
        ...
        "edition_count": 55,
        "subjects": ["Dystopian", "Classics", "Science Fiction", "Philosophy", "Literature"],
    },
]
...
class RecommendationService:
    ...
    def score_book_for_user(
        ...
        score = 5.0  # Base positive candidate score
        reasons: List[str] = []
        ...
        book_subjects = [s.strip().lower() for s in (book.subjects or []) if s]

        matched_profile_genres = [
            g for g in preferred_genres if any(g in s or s in g for s in book_subjects)
        ]
        if matched_profile_genres:
            score += len(matched_profile_genres) * 20.0
            reasons.append(
                f"Matches your preferred topics: {', '.join(matched_profile_genres[:3]).title()}"
            )

        # 2. Reading History Genre Similarity
        matched_history_genres = [
            g for g in user_favorite_genres if any(g in s or s in g for s in book_subjects)
        ]
        if matched_history_genres and not matched_profile_genres:
            score += len(matched_history_genres) * 12.0
            reasons.append(
                f"Similar to books you completed: {', '.join(matched_history_genres[:2]).title()}"
            )
        ...
        book_authors = [a.strip().lower() for a in (book.authors or []) if a]

        matched_profile_authors = [
            a for a in preferred_authors if any(a in ba or ba in a for ba in book_authors)
        ]
        if matched_profile_authors:
            score += 30.0
            reasons.append(
                f"Written by your preferred author: {', '.join(book.authors)}"
            )

        matched_history_authors = [
            a for a in user_favorite_authors if any(a in ba or ba in a for ba in book_authors)
        ]
        if matched_history_authors and not matched_profile_authors:
            score += 15.0
            reasons.append(f"By an author you enjoyed: {', '.join(book.authors)}")
    ...
    async def get_recommendations(
        ...
        user_bookshelf = await bookshelf_repository.list_by_user(db, current_user.id)
        excluded_book_ids = {item.book_id for item in user_bookshelf}
        ...
        for item in user_bookshelf:
            if item.status == "COMPLETED" and item.book:
                for subj in item.book.subjects or []:
                    user_favorite_genres.add(subj.strip().lower())
                for auth in item.book.authors or []:
                    user_favorite_authors.add(auth.strip().lower())

        # 4. Query candidate books
        stmt = select(Book)
        if excluded_book_ids:
            stmt = stmt.where(Book.id.notin_(excluded_book_ids))
        stmt = stmt.limit(100)

        result = await db.execute(stmt)
        candidate_books = list(result.scalars().all())

        # If candidates are empty (e.g. all books in shelf), query all books
        ...
        # Sort descending by score, tie-break by average rating and title
        scored_items.sort(
            key=lambda x: (x.score, x.average_rating, x.title),
            reverse=True,
        )
